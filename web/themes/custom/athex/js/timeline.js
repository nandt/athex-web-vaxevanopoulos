...
window.addEventListener('scroll', function() {
    ...
    const triggerHeight = window.innerHeight * 0.75; // ή όποιο σημείο θέλετε

    bullets.forEach(function(bullet) {
        const bulletPosition = bullet.getBoundingClientRect().top;

        if (bulletPosition < triggerHeight) {
...
document.addEventListener("DOMContentLoaded", function() {
    window.addEventListener('scroll', function() {
        ...
        if (window.scrollY > 50) { // Αντικαταστήστε το 50 με το ύψος στο οποίο θέλετε να αλλάξει το χρώμα
            verticalBar.classList.add('scrolled');
        } else {
            verticalBar.classList.remove('scrolled');
        }
    });
});
